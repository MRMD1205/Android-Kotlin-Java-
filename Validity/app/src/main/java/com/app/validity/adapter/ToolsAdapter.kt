...
class ToolsAdapter(val dashboardList: ArrayList<DashboardList>,
                   val itemClickCallback: ItemClickCallback<DashboardList>) :
        RecyclerView.Adapter<ToolsAdapter.ObjectHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ObjectHolder {
        ...
        return ObjectHolder(
                LayoutInflater.from(context).inflate(R.layout.list_item_tools, parent, false)
        )
    }
    ...
    class ObjectHolder(view: View) : RecyclerView.ViewHolder(view) {
        ...
        fun bindItems(dashboardItem: DashboardList, itemClickCallback: ItemClickCallback<DashboardList>) {
            val txtUserName: TextView = itemView.findViewById(R.id.txtVehicalCategoryName) as TextView
            val imgProfile: AppCompatImageView = itemView.findViewById(R.id.ivVehicalCategory) as AppCompatImageView
