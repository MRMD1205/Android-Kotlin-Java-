...
class VehicalAccidentListAdapter(
    val dashboardList: ArrayList<VehicalAccident>,
    val itemClickCallback: ItemClickCallback<VehicalAccident>
) :
    RecyclerView.Adapter<VehicalAccidentListAdapter.ObjectHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ObjectHolder {
        ...
        return ObjectHolder(
            LayoutInflater.from(context).inflate(R.layout.list_item_accident, parent, false)
        )
    }
    ...
    class ObjectHolder(view: View) : RecyclerView.ViewHolder(view) {
        ...
        fun bindItems(
            item: VehicalAccident,
            itemClickCallback: ItemClickCallback<VehicalAccident>
        ) {
            ...
            val strDate = if (TextUtils.isEmpty(item.accidentDate)) " - " else item.accidentDate
