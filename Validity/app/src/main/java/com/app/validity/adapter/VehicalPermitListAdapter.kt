...
class VehicalPermitListAdapter(
    val dashboardList: ArrayList<VehicalPermit>,
    val itemClickCallback: ItemClickCallback<VehicalPermit>
) :
    RecyclerView.Adapter<VehicalPermitListAdapter.ObjectHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ObjectHolder {
        ...
        return ObjectHolder(
            LayoutInflater.from(context).inflate(R.layout.list_item_permit, parent, false)
        )
    }
    ...
    class ObjectHolder(view: View) : RecyclerView.ViewHolder(view) {
        ...
        fun bindItems(
            item: VehicalPermit,
            itemClickCallback: ItemClickCallback<VehicalPermit>
        ) {

            val txtPolicyValue: TextView = itemView.findViewById(R.id.txtPolicyValue) as TextView
            txtPolicyValue.text = if (TextUtils.isEmpty(item.permitNumber)) " - " else item.permitNumber
