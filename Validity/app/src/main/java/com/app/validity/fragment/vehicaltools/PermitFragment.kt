...
import com.app.validity.adapter.VehicalPermitListAdapter
import com.app.validity.model.BaseResponse
import com.app.validity.model.GetVehiclePermitListResponse
import com.app.validity.model.VehicalPermit
import com.app.validity.util.ADD_PERMIT_FRAGMENT
...
import com.app.validity.util.VIEW_PERMIT_FRAGMENT
...
class PermitFragment(var vehicalId: String) : BaseFragment(), ItemClickCallback<VehicalPermit> {

    val listPermitType: MutableList<String> = ArrayList()

    override fun setContentView(): Int = R.layout.fragment_permit
    ...
    override fun setListeners() {
        fabPermit.setOnClickListener {
            (requireActivity() as DashboardActivity).navigateToFragment(
                AddPermitFragment(vehicalId, null, listPermitType), true, ADD_PERMIT_FRAGMENT
            )
        }
    ...
    private fun callSetReminderAPI(date: String) {
        ...
        apiCallMethods.storeVehiclePermitsReminder(vehicalId, date, object : OnApiCallCompleted<Boolean> {
    ...
    override fun onResume() {
        super.onResume()
        getVehicleList()
        (requireActivity() as DashboardActivity).setHeader(R.string.menu_permit, false)
    }

    //API call
    private fun getVehicleList() {
        val apiCallMethods: ApiCallMethods = ApiCallMethods(requireActivity())
        apiCallMethods.getVehiclePermits(
            vehicalId,
            object : OnApiCallCompleted<GetVehiclePermitListResponse> {
                override fun apiSuccess(obj: Any?) {
                    val response: GetVehiclePermitListResponse =
                        obj as GetVehiclePermitListResponse
                    listPermitType.clear()
                    listPermitType.addAll(response.listPermitType)
    ...
    private fun setAdapter(vehicleListResponse: GetVehiclePermitListResponse) {
        var vehicalPermitListAdapter: VehicalPermitListAdapter =
            VehicalPermitListAdapter(
                vehicleListResponse.list as ArrayList<VehicalPermit>
                , this
            )
        rvPermit.adapter = vehicalPermitListAdapter
    }

    override fun onItemClick(view: View, selectedItem: VehicalPermit, position: Int) {

        when (view.id) {
            R.id.llView -> {
                (requireActivity() as DashboardActivity).navigateToFragment(
                    ViewPermitFragment(vehicalId, selectedItem), true, VIEW_PERMIT_FRAGMENT
                )
            }

            R.id.llEdit -> {
                (requireActivity() as DashboardActivity).navigateToFragment(
                    AddPermitFragment(vehicalId, selectedItem, listPermitType), true, ADD_PERMIT_FRAGMENT
                )
            }
    ...
    override fun onItemLongClick(view: View, selectedItem: VehicalPermit, position: Int) {

    }

    private fun callDeleteToolAPI(toolID: String) {
        val apiCallMethods: ApiCallMethods = ApiCallMethods(requireActivity())
        apiCallMethods.deletePermits(vehicalId, toolID, object : OnApiCallCompleted<BaseResponse> {
            override fun apiSuccess(obj: Any?) {
