...
class IndustrialAdapter(val list: ArrayList<GetIndustrialListResponse.Datum>,
                        val itemClickCallback: ItemClickCallback<GetIndustrialListResponse.Datum>) :
    RecyclerView.Adapter<IndustrialAdapter.ObjectHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ObjectHolder {
        ...
        return ObjectHolder(
            LayoutInflater.from(context).inflate(R.layout.list_item_industry, parent, false)
        )
    }
    ...
    class ObjectHolder(view: View) : RecyclerView.ViewHolder(view) {
        ...
        fun bindItems(item: GetIndustrialListResponse.Datum,
                      itemClickCallback: ItemClickCallback<GetIndustrialListResponse.Datum>) {
            val txtPolicyValue: TextView = itemView.findViewById(R.id.txtPolicyValue) as TextView
            txtPolicyValue.text = item.industryType?.name.toString()
            ...
            val strDate = if (TextUtils.isEmpty(item.purchaseDate)) " - " else item.purchaseDate
