...
class ShowImagesNoClickAdapter(val list: MutableList<String?>) : RecyclerView.Adapter<ShowImagesNoClickAdapter.ObjectHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ObjectHolder {
        return ObjectHolder(LayoutInflater.from(parent.context).inflate(R.layout.list_item_selected_images, parent, false))
    }
    ...
    class ObjectHolder(view: View) : RecyclerView.ViewHolder(view) {
        ...
        fun bindItems(item: String?) {
            ...
            if (item == null) {
                ...
                Glide.with(itemView.context).load(item).apply(RequestOptions().placeholder(R.drawable.ic_loading).error(R.drawable.ic_image_unavailable)).into(ivImage)
            }
        }
    }
}