...
class PersonalDocumentAdapter(val list: ArrayList<GetPersonalDocumentListResponse.Datum>,
                              val itemClickCallback: ItemClickCallback<GetPersonalDocumentListResponse.Datum>) :
    RecyclerView.Adapter<PersonalDocumentAdapter.ObjectHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ObjectHolder {
        ...
        return ObjectHolder(
            LayoutInflater.from(context).inflate(R.layout.list_item_personal_document, parent, false)
        )
    }
    ...
    class ObjectHolder(view: View) : RecyclerView.ViewHolder(view) {
        ...
        fun bindItems(item: GetPersonalDocumentListResponse.Datum,
                      itemClickCallback: ItemClickCallback<GetPersonalDocumentListResponse.Datum>) {
            val txtPolicyValue: TextView = itemView.findViewById(R.id.txtPolicyValue) as TextView
            txtPolicyValue.text = if (TextUtils.isEmpty(item.name)) " - " else item.name
            ...
            val strDate = if (TextUtils.isEmpty(item.startDate)) " - " else item.startDate
            ...
            val endDate = if (TextUtils.isEmpty(item.endDate)) " - " else item.endDate
