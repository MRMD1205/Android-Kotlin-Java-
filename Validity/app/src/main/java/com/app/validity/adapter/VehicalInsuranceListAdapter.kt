...
class VehicalInsuranceListAdapter(
    val dashboardList: ArrayList<VehicalInsurance>,
    val itemClickCallback: ItemClickCallback<VehicalInsurance>
) :
    RecyclerView.Adapter<VehicalInsuranceListAdapter.ObjectHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ObjectHolder {
        ...
        return ObjectHolder(
            LayoutInflater.from(context).inflate(R.layout.list_item_insurance, parent, false)
        )
    }
    ...
    class ObjectHolder(view: View) : RecyclerView.ViewHolder(view) {
        ...
        fun bindItems(
            item: VehicalInsurance,
            itemClickCallback: ItemClickCallback<VehicalInsurance>
        ) {

            val txtPolicyValue: TextView = itemView.findViewById(R.id.txtPolicyValue) as TextView
            txtPolicyValue.text = if (TextUtils.isEmpty(item.policyNumber)) " - " else item.policyNumber
