...
class OthersListAdapter(
    val dashboardList: ArrayList<GetOthersListResponse.Datum>,
    val itemClickCallback: ItemClickCallback<GetOthersListResponse.Datum>
) :
    RecyclerView.Adapter<OthersListAdapter.ObjectHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ObjectHolder {
        ...
        return ObjectHolder(
            LayoutInflater.from(context).inflate(R.layout.list_item_others, parent, false)
        )
    }
    ...
    class ObjectHolder(view: View) : RecyclerView.ViewHolder(view) {
        ...
        fun bindItems(
            item: GetOthersListResponse.Datum,
            itemClickCallback: ItemClickCallback<GetOthersListResponse.Datum>
        ) {

            val txtTitle: TextView = itemView.findViewById(R.id.txtTitle) as TextView
            txtTitle.text = if (TextUtils.isEmpty(item.name)) " - " else item.name
